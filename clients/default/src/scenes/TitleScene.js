...
export default class TitleScene extends PIXI.Container {
  constructor() {
    ...
    // this.room.on('error', console.error)
    ...
    if (!patches) {
      ...
      for (let i=0; i < patches.length; i++) {
        ...
        if (patch.op === "add" && entityId) {
          ...
          patch.path.match(/\/entities\/([a-zA-Z0-9_-]+)$/)
        ) {
  ...
  setupMap(state) {
    ...
    // add entity elements
    // TODO: replace for in with forEach
  ...
  createEntity(entityId, data) {
    ...
    if (!entity) {
      return console.warn("couldn't create entity. ", data)
    }
